/**
 * ATOMS3_EnvIV_Monitor.ino
 * ATOMS3 + Unit EnvIV (SHT40 + BMP280)
 * 
 * Features:
 * - EnvIV (I2C) for Temp, Humidity, Pressure
...
void setup() {
    ...
    Serial.println("EnvIV Initialized");
...
void drawSeries(float* data, uint16_t color, float plotMin, float plotMax, int pointRadius, int labelOffsetY, bool drawLabels) {
    ...
    if (drawLabels) {
        ...
        // Note: Bottom labels grow upwards (reverse offset) to avoid overlapping container bottom? 
        // Actually, let's keep consistent offset from bottom anchor.
        // If labelOffsetY is 0 (Temp), it's at bottom.
        // If labelOffsetY is 8 (Hum), it's 8px above bottom.
        // If labelOffsetY is 16 (Press), it's 16px above bottom.
    }
}
...
void drawGraph() {
    ...
    // Offsets to stack labels: 0, 8, 16 pixels
    // Pass ranges explicitly
    
    // Clearing the labels area first? 
    // Since drawScreen clears everything, we are good.
...
void loop() {
    ...
    if (millis() - lastMeasureTime >= MEASURE_INTERVAL) {
        ...
        // Measure
        if (sht4.update()) {
            currentData.temp = sht4.cTemp;
            currentData.hum = sht4.humidity;
        }
        if (bmp.update()) {
            currentData.press = bmp.pressure / 100.0; // Pa -> hPa
        }
