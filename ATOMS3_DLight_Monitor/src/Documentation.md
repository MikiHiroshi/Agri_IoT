...
ATOMS3と照度センサーユニット（Unit DLight）を使って、照度を測定し、クラウド（Ambient）へ送信しつつ、手元の画面でリアルタイムグラフを確認できるプログラムです。
...
ATOMS3の小ささを活かしつつ、画面には必要な情報（現在値と履歴）を見やすく配置しました。経時的な照度の変化をディスプレイ内で確認できるのが便利だと思います。
現在横軸は1分1プロットで90分間の時間幅となっていますが、プログラムを変更することで横軸の時間幅を24時間などにもできます。好みに合わせてプログラム修正してください。
